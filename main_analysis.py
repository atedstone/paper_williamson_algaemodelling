...
def get_site_daily_timeseries(infile, x, y, outfile):
    """ As input, expects netcdf file generated by calculate_qmc_daily_MIQR.py
    Used for site-specific validation/comparison against other (observed) populations
    """
...
# %% [markdown]
# ---
# ## Fig. 3: QMC parameters distribution

# %% trusted=true
expts = pd.read_csv(EXPTS_DESIGN_FN, index_col=0)

# %% trusted=true
fig, ax = plt.subplots(2,2, figsize=(2.4,2.4))
ax[0,0].hist(expts['ploss']*100, color='#D86ECC')
ax[0,0].set_xlabel('% loss')
ax[0,0].set_ylabel('n')
ax[0,1].hist(expts['T_celsius'], color='tab:orange')
ax[0,1].set_xlabel(r'Air temp. ($^oC$)')
ax[0,1].set_ylabel('n')
ax[1,0].hist(expts['snow_depth_metres'], color='tab:blue')
ax[1,0].set_xlabel('Snow (m)')
ax[1,0].set_ylabel('n')
ax[1,1].hist(expts['SWd_wm2'], color='tab:green')
ax[1,1].set_xlabel(r'SWD (W m$^{-2}$)')
ax[1,1].set_ylabel('n')
...
subset = get_site_daily_timeseries(
    os.path.join(RESULTS, 'model_outputs_2014_XYT_dailypop_MIQR.nc'), 
    pts_ps.loc['S6','geometry'].x,
    pts_ps.loc['S6','geometry'].y,
    os.path.join(RESULTS, 'QMC_algpop_daily_med_IQR_S6_2014.csv')
)

# %% trusted=true
plt.figure()
subset.med.plot(color='k', linewidth=2)
...
plt.fill_between(subset.index, subset.q25, subset.q75, alpha=0.5, color='tab:red')
plt.plot(subset.index, subset.med, color='tab:red')

plt.plot(stib_ts.date, stib_ts['observed.cells.ml.dw'], 'o', mfc='none', mec='gray', alpha=0.7)
stib_mean = stib_ts.groupby('date')['observed.cells.ml.dw'].mean()
stib_mean.name = 'stib_mean'
stib_med = stib_ts.groupby('date')['observed.cells.ml.dw'].median()
stib_med.name = 'stib_median'
stib_mean.plot(label='Obs. mean', color='k', linestyle='--')
stib_med.plot(label='Obs. median', color='k')

# Calculate and display R^2
comparison = pd.merge(stib_mean, subset, left_index=True, right_index=True, how='inner').dropna()
ols = sm.WLS(comparison.med, sm.add_constant(comparison.stib_mean), weights=1.0 / ((comparison.q75-comparison.q25) ** 2)).fit()
print(ols.summary())
ax.annotate(f'$R^2$ {ols.rsquared:.2f}\n$p$ {ols.pvalues["stib_mean"]:.2f}',
            xy=(0.96, 0.04), xycoords='axes fraction',
            horizontalalignment='right', verticalalignment='bottom', fontsize=7)

plt.legend()

plt.xlim(pd.Timestamp(2014,6,15), pd.Timestamp(2014,8,15))
...
dl = dates.DayLocator(interval=2) 
...
# %% trusted=true
meas_mod = mmdf[(mmdf['meas_sd_ng_dw_ml'] > 0) & (mmdf['n.samples'] >= 5)]
weights = 1 / (meas_mod['meas_sd_ng_dw_ml']**2)
m = sm.WLS(np.log10(meas_mod['modelled.50_ngdwml']), sm.add_constant(np.log10(meas_mod['meas_mean_ng_dw_ml'])), w=weights).fit() #, w=weights
print(m.summary())
...
# %% trusted=true
polys_wang = gpd.read_file(os.path.join(WORK_ROOT, 'glacier_algal_biomass_datasets/Figure3_site_extent/Sites_figure3.shp'))
polys_wang = polys_wang.to_crs(32622)
...
# %% trusted=true
# Download satellite image for use as location map of Wang locations.
...
im_output_path = os.path.join(WORK_ROOT, f'S2_{date1}_{date2}_RGB_openeo.tif')
if os.path.exists(output_path) and overwrite_im is False:
    print (' Already available on disk.')
elif not online:
    raise ConnectionError('Need to be online to continue')
else:
    dc = con.load_collection(
        "SENTINEL2_L2A",
        spatial_extent={"west": -51.0, "south": 65.5, "east": -48.7, "north": 70.0},
        temporal_extent = [date1, date2],
        bands=["B02", "B03", "B04"],
        max_cloud_cover=80
    )
    dc = dc.resample_spatial(resolution=500, method='cubic')
    #dc = dc.merge_cubes()
    dc = dc.reduce_dimension(dimension="t", reducer="median")
    
    dc.download(output_path)
    
im = gu.Raster(im_output_path)
...
# nrows, ncols
gs = fig.add_gridspec(2, 5, width_ratios=[0.1, 0.9/4, 0.9/4, 0.9/4, 0.9/4])
...
ax = plt.subplot(gs[:,0])
(im*0.0001).plot(ax=ax)
polys_wang.plot(ax=ax, color='none', edgecolor='yellow')
n = 1
pts_wang_32622 = pts_wang.to_crs(32622)
for ix, pt in pts_wang_32622.iterrows():
    ax.annotate(
        f'Site {n}',
        xy=(pt.geometry.x, pt.geometry.y),           # Point to annotate
        xytext=(-15,3),         # Offset of label from point (in points)
...
with pd.ExcelWriter(os.path.join(RESULTS, 'sfig3.xlsx')) as w:    
    ...
    for year in [2016, 2017]:
        for ix, row in pts_wang.iterrows():
            ...
            # # Calculate and display R^2
            # comparison = pd.merge(tsw, s, left_index=True, right_index=True, how='inner').dropna()
            # ols = sm.WLS(comparison.med, sm.add_constant(comparison.wang), weights=1.0 / ((comparison.q75-comparison.q25) ** 2)).fit()
            #print(ols.summary())
            # ax.annotate(f'$R^2$ {ols.rsquared:.2f}\n$p$ {ols.pvalues["wang"]:.2f}',
            #             xy=(0.96, 0.04), xycoords='axes fraction',
            #             horizontalalignment='right', verticalalignment='bottom', fontsize=7)
...
# %% [markdown]
# ## Fig. 6: Map small versus large bloom years
...
# %% [markdown]
# ## Fig. 7: Time-series / Sector-by-sector analysis
...
# All sectors SMB
axr = ax.twinx()
# Subtract a year from the SMB index. This is to align it with summertime - the SMB index provided is
# just year, so e.g. 2019. Above, Pandas reads this in as 2019-01-01. But actually, according to the 
# PROMICE code, 2019 will refer to the SMB year beginning 2019-09-01 and run through to 2020-08-31.
# Therefore SMB value to show next to blooming 2019 is for 2018-2019 MB year, indexed at 2018.
...
# %% [markdown]
# ## Fig. 8: Extent of blooms / % coverage of ice sheet by blooms and trend analysis
...
# %% trusted=true
qmc_med.sel(x=x, y=y, method='nearest').annual_pop_max.plot()
...
plt.plot(x, y, '+', markersize=9, color='red')
plt.plot(ulx, uly, '+', color='orange')
plt.plot(lrx, lry, '+', color='orange')

# %% trusted=true
store = []
for qm in [qmc_q25, qmc_med, qmc_q75]:
    ts = qm.sel(x=slice(ulx, lrx), y=slice(lry, uly)).annual_pop_max \
        .squeeze().stack({'xy':['x', 'y']}).to_pandas().median(axis=1)
    store.append(ts)
df = pd.concat(store, axis=1)
df.columns = ['q25', 'q50', 'q75']

# %% trusted=true
df.to_csv(os.path.join(RESULTS, f'qanaaqicecap_pmax_spatialmedian.csv'))
